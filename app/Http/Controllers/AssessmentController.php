...
class AssessmentController extends Controller
{
    public function submitLatihan(Request $request, Module $module)
    {
        $questions = $module->questions()
            ->where('type', 'latihan')
            ->get();
        [$score, $correct, $answers] = $this->score(
            $questions,
    ...
    public function quizIndex()
    {
        $modules = Module::where('is_active', true)
            ->orderBy('order')
            ->get();
        $results = AssessmentResult::where('session_id', session()->getId())
            ->where('type', 'quiz')
    ...
    public function quizShow(Module $module)
    {
        ...
        $done = AssessmentResult::where('session_id', session()->getId())
        ...
        if ($done) {
            return redirect()
                ->route('quiz.index')
                ->with('warning', 'Quiz modul ini sudah pernah dikerjakan. Nilai tersimpan: ' . $done->score);
        }
    ...
    {
        $this->ensureUnlocked($module);
        $alreadyDone = AssessmentResult::where('session_id', session()->getId())
            ->where('module_id', $module->id)
            ->where('type', 'quiz')
            ->exists();
        if ($alreadyDone) {
            return redirect()
                ->route('quiz.index')
                ->with('warning', 'Quiz hanya dapat dikerjakan satu kali.');
        }
    ...
    public function evaluasiShow()
    {
        ...
        if ($alreadyDone) {
            return back()
                ->with('warning', 'Evaluasi akhir sudah pernah dikirim. Nilai tidak ditampilkan dan tersimpan di database.');
        }

        $questions = Question::where('type', 'evaluasi')
            ->with('module')
            ->get();
        return view('assessment.form', [
            'mode' => 'evaluasi',
    ...
    public function evaluasiSubmit(Request $request)
    {
        ...
        if ($alreadyDone) {
            return redirect()
                ->route('home')
                ->with('warning', 'Evaluasi hanya dapat dikirim satu kali.');
        }
    ...
    {
        $previousModule = Module::where('order', '<', $module->order)
            ->orderByDesc('order')
            ->first();
        ...
        $isCompleted = ModuleProgress::where('session_id', session()->getId())
            ->where('module_id', $previousModule->id)
            ->where('latihan_completed', true)
            ->exists();

        if (!$isCompleted) {
            abort(403, 'Modul terkunci. Selesaikan latihan modul sebelumnya.');
        }
    }
}